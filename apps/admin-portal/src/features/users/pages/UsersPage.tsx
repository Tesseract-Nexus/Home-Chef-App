                    ...
                    <td className="px-6 py-4">
                      ...
                      {user.lastOrderAt && (
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Last: {formatRelative(user.lastOrderAt)}
                        </p>
                      )}
                    </td>
                    <td className="px-6 py-4">
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-6 py-3">
            <p className="text-sm text-muted-foreground">
              ...
              {Math.min(pagination.page * pagination.limit, Number(pagination.total))} of{' '}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-lg border border-border px-3 py-1.5 text-sm text-foreground hover:bg-secondary disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </button>
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${styles[role] || 'bg-muted text-muted-foreground'}`}>
      {role.replace('_', ' ')}
    </span>
  );
}
