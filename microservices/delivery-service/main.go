...
func seedMockData(db *gorm.DB, logger *zap.Logger) error {
	...
	var count int64
	db.Model(&models.DeliveryPartner{}).Count(&count)
	if count > 0 {
		logger.Info("Mock data already exists, skipping seed")
		return nil
	}

	logger.Info("Seeding mock delivery data...")
	...
	partner := models.DeliveryPartner{
		...
		Documents: models.DocumentInfo{
			...
			DrivingLicenseExpiry:        time.Now().AddDate(5, 0, 0),
	...
	if err := db.Create(&partner).Error; err != nil {
		return fmt.Errorf("failed to create delivery partner: %w", err)
	}

	// Create mock earnings
	earnings := []models.DeliveryEarning{
		{
			ID:                "earning-1",
			DeliveryPartnerID: "delivery-partner-1",
			OrderID:           "order-1",
	...
	logger.Info("Mock delivery data seeded successfully")
	return nil
}
