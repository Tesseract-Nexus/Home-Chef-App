package routes

import (
	"rewards-service/config"
	"rewards-service/handlers"
	"rewards-service/middleware"
	"rewards-service/services"
	"rewards-service/websocket"

	"github.com/gin-gonic/gin"
	"go.uber.org/zap"
	"gorm.io/gorm"
)

func SetupRoutes(cfg *config.Config, db *gorm.DB, hub *websocket.Hub, logger *zap.Logger) *gin.Engine {
	// Set Gin mode
	if cfg.Environment == "production" {
		gin.SetMode(gin.ReleaseMode)
	}

	router := gin.New()

	// Middleware
	router.Use(middleware.CORSMiddleware())
	router.Use(middleware.LoggerMiddleware(logger))
	router.Use(gin.Recovery())

	// Health check
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{
			"status":    "healthy",
			"service":   "rewards-service",
			"websocket": gin.H{
				"clients": hub.GetConnectedClients(),
			},
		})
	})

	// Initialize services
	rewardsService := services.NewRewardsService(db, logger)
	subscriptionService := services.NewSubscriptionService(db, logger)

	// Initialize handlers
	rewardsHandler := handlers.NewRewardsHandler(rewardsService)
	subscriptionHandler := handlers.NewSubscriptionHandler(subscriptionService)

	// WebSocket endpoint
	router.GET("/ws", hub.HandleWebSocket)

	// API routes
	api := router.Group("/v1")
	
	// Public routes (no auth required)
	public := api.Group("")
	{
		// Subscription plans (public access)
		public.GET("/subscriptions", subscriptionHandler.GetSubscriptionPlans)
	}
	
	// Protected routes (require authentication)
	api.Use(middleware.AuthMiddleware(cfg))
	{
		// Rewards Management
		api.GET("/rewards/profile", rewardsHandler.GetRewardsProfile)
		api.POST("/rewards/earn", rewardsHandler.EarnTokens)
		api.POST("/rewards/redeem", rewardsHandler.RedeemTokens)
		api.GET("/rewards/transactions", rewardsHandler.GetRewardTransactions)

		// Subscription Management
		api.POST("/subscriptions", subscriptionHandler.CreateSubscription)
		api.GET("/subscriptions/current", subscriptionHandler.GetCurrentSubscription)
		api.PUT("/subscriptions/current", subscriptionHandler.UpdateSubscription)
		api.DELETE("/subscriptions/current", subscriptionHandler.CancelSubscription)
	}

	return router
}