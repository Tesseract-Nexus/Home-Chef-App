...
func (h *Hub) Run() {
	for {
		...
		case client := <-h.register:
			...
			h.mutex.Unlock()
			h.logger.Info("Search client connected", zap.String("user_id", client.userID))
		...
		case client := <-h.unregister:
			...
			if _, ok := h.clients[client]; ok {
				...
				close(client.send)
				h.logger.Info("Search client disconnected", zap.String("user_id", client.userID))
			}
			h.mutex.Unlock()
...
func (c *Client) handleMessage(msg *Message) {
	...
	default:
		c.hub.logger.Info("Received search message", zap.String("event", msg.Event), zap.String("user_id", c.userID))
	}
}

func (h *Hub) BroadcastSearchUpdate(message *Message) {
