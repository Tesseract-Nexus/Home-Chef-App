...
type Client struct {
	...
	// User type (admin, developer, etc.)
	userType string
}

// Message types
const (
	MessageTypeWebhookDelivery = "webhook_delivery"
	MessageTypeWebhookFailed   = "webhook_failed"
	MessageTypeWebhookRetry    = "webhook_retry"
)
	...
	for {
		...
		case client := <-h.register:
			...
			welcome := WebSocketMessage{
				Type:      "connected",
				Data:      map[string]string{"message": "Connected to webhook service"},
				Timestamp: time.Now(),
			}
...
// BroadcastWebhookDelivery notifies about webhook delivery status
func (h *Hub) BroadcastWebhookDelivery(userID string, delivery *models.WebhookDelivery) {
	message := WebSocketMessage{
		Type: MessageTypeWebhookDelivery,
...
// BroadcastWebhookFailed notifies about webhook delivery failure
func (h *Hub) BroadcastWebhookFailed(userID string, delivery *models.WebhookDelivery) {
	message := WebSocketMessage{
		Type: MessageTypeWebhookFailed,
		Data: map[string]interface{}{
			"delivery_id":   delivery.ID,
			"webhook_id":    delivery.WebhookID,
			"event_type":    delivery.EventType,
